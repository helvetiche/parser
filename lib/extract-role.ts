...
export type RoleExtractionResult = {
  role: RoleData;
  usage: TokenUsage;
  model: string;
};
...
export async function extractRole(text: string, model: string = ROLE_MODEL): Promise<RoleData> {
  const { role } = await extractRoleWithUsage(text, model);
  return role;
}

export async function extractRoleWithUsage(
  text: string,
  model: string = ROLE_MODEL
  ...
  return { role: roleFromUnknown(data), usage, model: usedModel };
}
